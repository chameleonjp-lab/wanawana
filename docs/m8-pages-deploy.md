...
公開前の確認では、GitHubリポジトリのSettings → PagesでSourceをGitHub Actionsに設定する。初回はActionsの `Deploy GitHub Pages` を手動実行できる。公開先は `/wanawana/`。1.0ではService Workerを登録・同梱せず、オフライン起動を約束しない。
